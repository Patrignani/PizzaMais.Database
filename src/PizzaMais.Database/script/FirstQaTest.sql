...
insert into City([Name],Active, DateCreation, UserIdCreation) values('Jundiaí/SP', 1, GETDATE(), 1) 
    ...
    BEGIN
			 ...
			 insert into Phone(Number,PhoneType,Active, DateCreation, UserIdCreation) OUTPUT INSERTED.ID  INTO @PhoneId(ID) values(999970000 + @loop, 1, 1, GETDATE(), 1) 
			  insert into Andress(Cep,District, street, CityId,Active, DateCreation, UserIdCreation) OUTPUT INSERTED.ID  INTO @AndressId(ID)
			  values(13213000 + @loop,'Vila '+  CONVERT(varchar(10),@loop), 'Rua '+  CONVERT(varchar(10),@loop),3,  1, GETDATE(), 1) 
...
WHILE @loop < @Count   
	...
	set @CustomerId = (SELECT Id FROM CUSTOMER ORDER BY 1 OFFSET @loop ROWS FETCH NEXT 1 ROWS ONLY)
	 ...
	 insert into Phone(Number,PhoneType,Active, DateCreation, UserIdCreation) OUTPUT INSERTED.ID  INTO @PhoneId(ID) values(45980000 + @loop, 1, 1, GETDATE(), 1) 
